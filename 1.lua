...
local colors = {
    primary = Color3.fromRGB(255, 105, 0),
    secondary = Color3.fromRGB(255, 135, 30),
    accent = Color3.fromRGB(255, 165, 60),
    success = Color3.fromRGB(34, 197, 94),
    bg1 = Color3.fromRGB(26, 13, 5),
    bg2 = Color3.fromRGB(38, 19, 8),
    bg3 = Color3.fromRGB(53, 27, 12),
    bg4 = Color3.fromRGB(70, 36, 16),
    text = Color3.fromRGB(255, 250, 245),
    textDim = Color3.fromRGB(224, 205, 190),
...
-- Key AddConnection HARUS unik per komponen. Jangan pakai tostring(instance):
-- di Roblox itu mengembalikan .Name, bukan alamat unik, jadi dua komponen dengan
-- judul sama akan saling men-disconnect koneksi (komponen pertama mati diam-diam).
function Library:_nextConnId()
    self._connSeq = (self._connSeq or 0) + 1
...
function Library:Cleanup()
    ...
    if self._initWatchdog then
        pcall(function() task.cancel(self._initWatchdog) end)
    ...
    -- naikkan generasi dulu supaya kartu yang sedang ditutup tidak memanggil antrian berikutnya
    self._notifGen = (self._notifGen or 0) + 1
    if self._notifQueue then table.clear(self._notifQueue) end
    local currentNotif = self._notifCurrent
    self._notifCurrent = nil
    if currentNotif then
        currentNotif.closing = true
        if currentNotif.conn then pcall(function() currentNotif.conn:Disconnect() end) end
        pcall(function()
            if currentNotif.card and currentNotif.card.Parent then currentNotif.card:Destroy() end
        end)
    end
...
local function RegisterCallback(configPath, callback, componentType, defaultValue, updateVisualFn)
    ...
    if CallbackRegistry[configPath] then
        warn(("[LynxGUI] Judul komponen duplikat -> config path '%s' dipakai lebih dari sekali. Nilai config akan saling menimpa; pakai judul yang unik."):format(tostring(configPath)))
    end
...
local function safeInvoke(context, fn, value)
    local ok, err = pcall(fn, value)
    if not ok then
        warn(("[LynxGUI] %s error: %s"):format(context, tostring(err)))
    end
    return ok
end
local function ExecuteConfigCallbacks()
    for _, entry in pairs(CallbackRegistry) do
        if entry.updateVisual then
            ...
            safeInvoke("updateVisual @" .. tostring(entry.path), entry.updateVisual, value)
        end
    end
    local function runCallbacks(wantToggle)
        for _, entry in pairs(CallbackRegistry) do
            ...
            if entry.callback and isToggle == wantToggle then
                local value = Library.ConfigSystem.Get(entry.path, entry.default)
                safeInvoke("callback @" .. tostring(entry.path), entry.callback, value)
            end
        end
    end
    runCallbacks(false)
...
function Library:CreateWindow(config)
    ...
    local function createMinimizedIcon()
        ...
        local iconConns = {}       -- koneksi tetap (event pada instance ikon)
        local iconDragConns = {}   -- koneksi UIS global; HANYA hidup selama drag
        ...
        local function stopIconDrag()
            iconDragging = false
            disconnectList(iconDragConns)
        end
        local function disconnectIconConns()
            stopIconDrag()
            disconnectList(iconConns)
        end
        ...
        local function beginIconDrag(input)
            ...
            iconDragConns[#iconDragConns + 1] = UserInputService.InputChanged:Connect(function(moveInput)
                ...
                if moveInput.UserInputType == Enum.UserInputType.MouseMovement or moveInput.UserInputType == Enum.UserInputType.Touch then
                    local delta = moveInput.Position - iconDragStart
            ...
            iconDragConns[#iconDragConns + 1] = UserInputService.InputEnded:Connect(function(endInput)
                if not iconDragging then return end
                if endInput.UserInputType == Enum.UserInputType.MouseButton1 or endInput.UserInputType == Enum.UserInputType.Touch then
                    local moved = iconDragMoved
                    local stillHasIcon = icon and icon.Parent
                    if stillHasIcon then
                        savedIconPos = icon.Position
                    end
                    stopIconDrag()
                    if stillHasIcon and not moved then
                        restoreFromIcon()
                    end
                end
            end)
...
function Library:CreatePage(name, title, imageId, order)
    if self.pages[name] then
        warn(("[LynxGUI] Nama tab duplikat -> '%s' dipakai lebih dari sekali. Tab lama akan tertimpa dan tombol navigasinya berhenti berfungsi; pakai nama tab yang unik."):format(tostring(name)))
    end
...
function Library:SetFirstPage(name, title)
    if title and self.pages[name] then
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    ...
    updateVisual()
    self:AddConnection(("toggle_%s_%d"):format(label, self:_nextConnId()), btn.MouseButton1Click:Connect(function()
        on = not on
        updateVisual()
        if configPath and not disableSave then
...
function Library:_showDropdown(dropdownContainer)
    ...
    self._openDropdown = dropdownContainer
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    local MIN_POOL = 12
    local MAX_POOL = 48   -- batas aman; jauh di atas kapasitas layar terbesar
    ...
    -- Untuk multi-select, Value adalah tabel milik dropdown ini sendiri (BUKAN referensi
    -- ke config atau ke tabel Default milik user). copyValue menjamin kita tak pernah
    -- berbagi referensi, jadi tidak ada mutasi tak sengaja ke config/Default/konsumen.
    ...
    -- Eksplisit, bukan `configPath and Get(...) or defaultValue`: pola itu salah
    -- mengembalikan defaultValue ketika nilai tersimpan kebetulan false/nil.
    local savedValue
    if configPath then
        savedValue = copyValue(Library.ConfigSystem.Get(configPath, defaultValue))
    else
        savedValue = copyValue(defaultValue)
    end
    local DropdownFunc = { Value = savedValue, Options = items }
    local allOptions = {}
    local filteredOptions = {}
    local selectedSet = {}
    local labelByValue = {}   -- lookup O(1); dulu nested-loop O(terpilih x opsi) tiap klik
    local rowPool = {}
    -- State per-row disimpan di tabel Lua, BUKAN atribut Instance: SetAttribute jauh
    -- lebih mahal (alokasi + memicu AttributeChanged) dan refreshVisible dipanggil
    -- tiap frame selama scroll.
    local rowIndex = {}       -- row -> index opsi yang sedang ditampilkan
    local rowPos = {}         -- row -> offset Y terakhir yang ditulis
    local rowOpt = {}         -- row -> opsi terakhir yang di-paint
    local rowSel = {}         -- row -> status terpilih terakhir yang di-paint
    local rowShown = {}       -- row -> nilai Visible terakhir yang ditulis
    local lastCanvasH = nil
    local searchThread = nil
    local needsRefresh = false
    local poolBuilt = false
    local optionsNormalized = false
    -- Status buka/tutup dipegang satu variabel di level Library, jadi tidak perlu
    -- satu koneksi GetPropertyChangedSignal("Visible") per dropdown.
    local function isDropdownOpen()
        return self._openDropdown == dropdownContainer
    end
    ...
    local function rebuildLabelMap()
        table.clear(labelByValue)
        for _, opt in ipairs(allOptions) do
            if labelByValue[opt.value] == nil then
                labelByValue[opt.value] = opt.label
            end
        end
    end
    ...
    -- Label ditampilkan maksimal beberapa item lalu diringkas. Tanpa ini, memilih
    -- ratusan opsi menghasilkan string sangat panjang yang tetap diukur penuh oleh
    -- TextLabel walau TextTruncate aktif.
    local MAX_LABELS_SHOWN = 3
    local function updateClosedLabel()
        ensureOptionsNormalized()
        if isMulti then
            local values = DropdownFunc.Value
            ...
            for _, v in ipairs(values) do
                local lbl = labelByValue[v]
                if lbl then
                    shown = shown + 1
                    if shown <= MAX_LABELS_SHOWN then
                        labels[#labels + 1] = lbl
                    end
                end
            end
            if shown == 0 then
                optionLabel.Text = defaultText
            elseif shown > MAX_LABELS_SHOWN then
                optionLabel.Text = ("%s (+%d)"):format(table.concat(labels, ", "), shown - MAX_LABELS_SHOWN)
            else
                optionLabel.Text = table.concat(labels, ", ")
            end
        else
            ...
            if v == nil then
                ...
                optionLabel.Text = labelByValue[v] or tostring(v)
            end
        end
    end
    local function paintRow(row, opt)
        local selected = selectedSet[opt.value] == true
        -- Saat scroll, mayoritas row cuma bergeser posisi: opsi & status terpilihnya
        -- sama. Lewati 5 property write itu kalau memang tidak ada yang berubah.
        if rowOpt[row] == opt and rowSel[row] == selected then return end
        rowOpt[row] = opt
        rowSel[row] = selected
        if selected then
    ...
    local function buildOneRow(i)
        ...
        row.MouseEnter:Connect(function()
            if rowIndex[row] then
                ...
                rowSel[row] = nil   -- warna sudah tidak sesuai state: batalkan cache paint
            end
        end)
        row.MouseLeave:Connect(function()
            local optIndex = rowIndex[row]
        ...
        btn.Activated:Connect(function()
            ...
            if isMulti then
                -- bangun array baru (jangan mutasi in-place -> hindari berbagi referensi)
                local newList = {}
                local removed = false
                for _, v in ipairs(DropdownFunc.Value) do
                    if v == value then
    ...
    local function growPoolTo(target)
        if target > MAX_POOL then target = MAX_POOL end
        for i = #rowPool + 1, target do
            buildOneRow(i)
        end
    end
    local function refreshVisible()
        ...
        -- Hanya tulis kalau berubah: menulis CanvasSize bisa meng-clamp CanvasPosition
        -- dan signal-nya memicu refreshVisible sekali lagi.
        ...
        -- pastikan pool cukup mengisi seluruh viewport (window bisa di-resize jadi besar)
        if maxVisible > #rowPool then growPoolTo(maxVisible) end
        ...
        for i = 1, #rowPool do
            ...
            if optIndex <= lastVisible then
                ...
                local y = (optIndex - 1) * ROW_STRIDE
                if rowPos[row] ~= y then
                    rowPos[row] = y
                    row.Position = UDim2.new(0, 0, 0, y)
                end
                paintRow(row, opt)
                rowIndex[row] = optIndex
                if rowShown[row] ~= true then
                    rowShown[row] = true
                    row.Visible = true
                end
            else
                if rowShown[row] ~= false then
                    rowShown[row] = false
                    row.Visible = false
                end
                rowIndex[row] = nil
    ...
    local function ensurePoolBuilt()
        ...
        ensureOptionsNormalized()
        local viewH = scrollSelect.AbsoluteSize.Y
        local initial = MIN_POOL
        if viewH > 0 then
            initial = math.max(MIN_POOL, math.floor(viewH / ROW_STRIDE) + 2)
        end
        growPoolTo(initial)
        ...
        self:AddConnection("dropdownResize_" .. dropdownLayoutOrder, scrollSelect:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            if isDropdownOpen() then refreshVisible() end
        end))
    end
function DropdownFunc:Clear()
        allOptions = {}
        filteredOptions = {}
        -- Ikut mereset `items` supaya Clear() benar-benar membuang semua state opsi.
        -- (Saat ini ensureOptionsNormalized sudah dipanggil konstruktor lewat
        -- updateClosedLabel, jadi ini defensif -- bukan perbaikan bug yang tercapai.)
        items = {}
        optionsNormalized = true
        table.clear(labelByValue)
    ...
    function DropdownFunc:AddOption(option)
        ...
        if labelByValue[value] == nil then
    ...
    function DropdownFunc:Set(Value)
        local stored = copyValue(Value)          -- salinan internal milik dropdown
        DropdownFunc.Value = stored
        rebuildSelectedSet()
        if configPath then
            Library.ConfigSystem.Set(configPath, copyValue(stored))   -- salinan terpisah untuk config
            MarkDirty()
        end
        updateClosedLabel()
        if isDropdownOpen() then refreshVisible() end
        if onSelect then
            if isMulti then
                onSelect(copyValue(stored))       -- kirim salinan ke konsumen
            else
                onSelect((stored ~= nil) and tostring(stored) or "")
            end
        end
    end
    function DropdownFunc:SetValue(val) self:Set(val) end
    function DropdownFunc:GetValue() return copyValue(self.Value) end
    function DropdownFunc:SetValues(newList, selecting)
        ...
        if isMulti then
            DropdownFunc.Value = (type(selecting) == "table") and copyValue(selecting) or {}
        else
            DropdownFunc.Value = (selecting ~= nil) and selecting or nil
        end
        rebuildSelectedSet()
        ...
        if onSelect then
            if isMulti then
                onSelect(copyValue(DropdownFunc.Value))
            else
    ...
    function DropdownFunc:Refresh(newList)
        ...
        if isMulti then
            ...
            -- Eksplisit, bukan `found and prevValue or nil`: kalau prevValue kebetulan
            -- false, pola itu salah membuangnya.
            if prevValue ~= nil and labelByValue[prevValue] ~= nil then
                DropdownFunc.Value = prevValue
            else
        ...
        if isDropdownOpen() then refreshVisible() end
        if configPath then
            Library.ConfigSystem.Set(configPath, copyValue(DropdownFunc.Value))
            MarkDirty()
        end
    end
    ...
    if configPath then
        ...
        -- bungkus onSelect supaya saat :Init() konsumen menerima nilai yang konsisten
        -- dengan pemanggilan runtime (salinan array untuk multi, string untuk single).
        local wrappedSelect = onSelect and function(val)
            if isMulti then
                onSelect(copyValue(val))
            else
                onSelect((val ~= nil) and tostring(val) or "")
            end
        end
        RegisterCallback(configPath, wrappedSelect, cbType, cbDef, function(val)
            DropdownFunc.Value = copyValue(val)
            rebuildSelectedSet()
            updateClosedLabel()
            if isDropdownOpen() then refreshVisible() end
        end)
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback, placeholder)
    ...
    local inputBox = new("TextBox", {
        ...
        PlaceholderText = (placeholder ~= nil and placeholder ~= "") and tostring(placeholder) or "Enter Value",
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.small,
        TextColor3 = colors.text,
        PlaceholderColor3 = colors.textDimmer,
    ...
    local function fireCallback(rawVal)
        if not callback then return end
        local v = rawVal
        if v == nil then v = "" end
        callback(resolveValue(v))
    end
    self:AddConnection(("input_%s_%d"):format(label, self:_nextConnId()), inputBox.FocusLost:Connect(function()
    ...
    if configPath then
        -- Input tersimpan: callback dijalankan sekali saat Library:Init() (bukan saat create)
        -- supaya tidak double-fire & konsisten dengan toggle/dropdown.
        RegisterCallback(configPath, function(val) fireCallback(val) end, "input", defaultValue, function(val)
            local shown = val
            if shown == nil then shown = defaultValue end
            inputBox.Text = (shown ~= nil) and tostring(shown) or ""
        end)
    else
        -- Input NoSave: tidak ikut :Init(), jadi jalankan callback sekali dari nilai default.
        fireCallback(initialValue)
    end
    local inputController = {
        ...
        set = function(val)
            ...
            inputBox.Text = (resolved ~= nil) and tostring(resolved) or ""
            if configPath then
...
function Library:CreateButton(parent, label, callback)
    ...
    self:AddConnection(("btn_click_%s_%d"):format(label, self:_nextConnId()), button.MouseButton1Click:Connect(function()
...
function Library:Initialize()
    ...
    -- Buat Config tab lebih dulu agar komponennya terdaftar sebelum aktivasi callback.
    ...
    -- Aktifkan semua fitur sesuai nilai config, tepat sekali. Error callback dilaporkan
    -- lewat warn (tidak ditelan diam-diam) sehingga tidak ada "visual ON tapi fitur OFF".
...
local NOTIFY_WIDTH     = isMobile and 250 or 290
local NOTIFY_MARGIN    = 14
local NOTIFY_MAX_QUEUE = 10
local NOTIFY_LOGO      = "rbxassetid://104332967321169"
...
-- Notifikasi tampil BERGANTIAN: hanya satu kartu di layar, notifikasi baru masuk antrian
-- dan muncul setelah kartu sebelumnya tertutup (tidak di-stack ke atas).
-- - Selama antrian berisi, timer kartu aktif berjalan lebih cepat agar antrian tidak lama.
-- - Notifikasi identik dengan yang sedang tampil / terakhir di antrian tidak diduplikasi
--   (kartu yang tampil cukup di-reset timernya), jadi aman kalau tombol di-spam.
-- - Di PC, hover kartu menjeda timer; tombol ✕ menutup kartu langsung.
function Library:MakeNotify(config)
    ...
    if #queue >= NOTIFY_MAX_QUEUE then
...
function Library:_showNextNotify()
    if self._notifCurrent then return end
    local queue = self._notifQueue
    ...
    -- ukuran kartu dihitung dari isi teks
    local W        = NOTIFY_WIDTH
    local PAD      = 12
    local bodyX    = 46
    local bodyW    = W - bodyX - PAD
    local headerY  = 9
    local headerH  = 16
    local brand    = self._windowTitle or "LynX"
    local brandW   = math.min(measureTextWidth(brand, fontSize.small, Enum.Font.GothamBold) + 2, 70)
    local titleMaxW = math.max(40, bodyW - (brandW + 14) - 48)
    local titleW   = math.min(measureTextWidth(item.title, fontSize.normal, Enum.Font.GothamBold) + 2, titleMaxW)
    local descH    = measureTextHeight(item.desc, fontSize.small, Enum.Font.GothamBold, bodyW)
    local contentH = measureTextHeight(item.content, fontSize.small, Enum.Font.Gotham, bodyW)
    local dividerY = headerY + headerH + 3
    local cursorY  = dividerY + 5
    local descY    = cursorY
    if descH > 0 then cursorY = cursorY + descH + 3 end
    local contentY = cursorY
    if contentH > 0 then cursorY = cursorY + contentH + 3 end
    local barY     = cursorY + 5
    local H        = barY + 2 + 10
    local shownPos  = UDim2.new(1, -NOTIFY_MARGIN, 1, -NOTIFY_MARGIN)
    local hiddenPos = UDim2.new(1, W + 30, 1, -NOTIFY_MARGIN)
    ...
    new("UICorner", {Parent = glow, CornerRadius = UDim.new(0, 7)})
    new("UIGradient", {
        Parent = glow,
        Transparency = NumberSequence.new({
    ...
    -- badge diamond: cincin luar berputar + diamond isi + logo.
    ...
    new("UIStroke", {Parent = ring, Color = color, Thickness = 1, Transparency = 0.65})
    ...
    new("UIStroke", {Parent = diamond, Color = color, Thickness = 1.5, Transparency = 0.1})
    new("ImageLabel", {
        Parent = card,
        Image = NOTIFY_LOGO,
        AnchorPoint = Vector2.new(0.5, 0.5),
    ...
    new("TextLabel", {
        Parent = card,
        Text = item.title,
        Size = UDim2.new(0, titleW, 0, headerH),
        Position = UDim2.new(0, bodyX, 0, headerY),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.normal,
        TextColor3 = color,
    ...
    local sepX = bodyX + titleW + 6
    local headerSep = new("Frame", {
        Parent = card,
        Size = UDim2.new(0, 1, 0, 12),
        Position = UDim2.new(0, sepX, 0, headerY + 2),
        BackgroundColor3 = color,
        BackgroundTransparency = 0.45,
        BorderSizePixel = 0,
        ZIndex = 204
    ...
    new("TextLabel", {
        Parent = card,
        Text = brand,
        Size = UDim2.new(0, brandW, 0, headerH),
        Position = UDim2.new(0, sepX + 7, 0, headerY),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.small,
        TextColor3 = colors.textDimmer,
    ...
    local queuePill = new("TextLabel", {
        Parent = card,
        AnchorPoint = Vector2.new(1, 0.5),
        Size = UDim2.new(0, 22, 0, 14),
        Position = UDim2.new(1, -(PAD + 20), 0, headerY + headerH / 2),
        BackgroundColor3 = colors.bg3,
        BackgroundTransparency = sectionTransparency,
        BorderSizePixel = 0,
        Text = "",
    ...
    new("UICorner", {Parent = queuePill, CornerRadius = UDim.new(1, 0)})
    new("UIStroke", {Parent = queuePill, Color = colors.border, Thickness = 1, Transparency = 0.4})
    local closeBtn = new("TextButton", {
        Parent = card,
        Size = UDim2.new(0, 22, 0, 22),
        Position = UDim2.new(1, -(PAD + 17), 0, headerY + headerH / 2 - 11),
        BackgroundTransparency = 1,
        Text = "",
        AutoButtonColor = false,
        ZIndex = 205
    ...
    for _, rot in ipairs({45, -45}) do
        closeLines[#closeLines + 1] = new("Frame", {
            Parent = closeBtn,
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = UDim2.new(0.5, 0, 0.5, 0),
    ...
    local function setCloseColor(c)
        for _, line in ipairs(closeLines) do
            line.BackgroundColor3 = c
        end
    end
    local divider = new("Frame", {
        Parent = card,
        Size = UDim2.new(0, bodyW, 0, 1),
        Position = UDim2.new(0, bodyX, 0, dividerY),
        BackgroundColor3 = colors.border,
    ...
    if descH > 0 then
        new("TextLabel", {
            Parent = card,
            Text = item.desc,
            Size = UDim2.new(0, bodyW, 0, descH),
            Position = UDim2.new(0, bodyX, 0, descY),
            BackgroundTransparency = 1,
            Font = Enum.Font.GothamBold,
            TextSize = fontSize.small,
            TextColor3 = colors.text,
    ...
    if contentH > 0 then
        new("TextLabel", {
            Parent = card,
            Text = item.content,
            Size = UDim2.new(0, bodyW, 0, contentH),
            Position = UDim2.new(0, bodyX, 0, contentY),
            BackgroundTransparency = 1,
    ...
    local track = new("Frame", {
        Parent = card,
        Size = UDim2.new(0, bodyW, 0, 2),
        Position = UDim2.new(0, bodyX, 0, barY),
    ...
    self._notifCurrent = state
    local spinTween = TweenService:Create(ring, TweenInfo.new(3, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1), {Rotation = 135})
    local hovered = false
    local lastPending = -1
    ...
    local function dismiss()
        ...
        state.closing = true
        stopTimers()
        local outTween = TweenService:Create(card, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = hiddenPos})
    ...
    card.Destroying:Connect(function()
        state.closing = true
        stopTimers()
        if self._notifCurrent ~= state then return end
        self._notifCurrent = nil
        task.delay(0.1, function()
    ...
    state.conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if pending ~= lastPending then
            lastPending = pending
            queuePill.Text = "+" .. pending
            queuePill.Visible = pending > 0
        end
        if not hovered then
            state.remaining = state.remaining - dt * (pending > 0 and 1.6 or 1)
        end
        fill.Size = UDim2.new(math.clamp(state.remaining / state.total, 0, 1), 0, 1, 0)
    ...
    closeBtn.MouseEnter:Connect(function() setCloseColor(color) end)
    closeBtn.MouseLeave:Connect(function() setCloseColor(colors.textDimmer) end)
    TweenService:Create(card, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = shownPos}):Play()
    spinTween:Play()
end
function Library:_createConfigTab(WindowObject)
    ...
    local resetBtnFrame
    -- Handle di-nil-kan begitu thread selesai; tanpa itu klik berikutnya memanggil
    -- task.cancel pada thread yang sudah mati -> error (ditelan pcall CreateButton,
    -- jadi tombolnya cuma terlihat "tidak berfungsi").
    local function cancelResetThread()
        if resetThread then
    ...
    local function cancelDeleteThread()
        if deleteThread then
            pcall(task.cancel, deleteThread)
            deleteThread = nil
        end
    end
...
function Library:Window(config)
    ...
    -- Watchdog: ingatkan kalau lupa memanggil Library:Init() di baris paling bawah script.
    if Library._initWatchdog then
        pcall(function() task.cancel(Library._initWatchdog) end)
    end
    Library._initWatchdog = task.delay(6, function()
        Library._initWatchdog = nil
        if not Library._initialized then
            warn("[LynxGUI] Library:Init() belum dipanggil. Tambahkan 'Library:Init()' di baris paling bawah script supaya toggle/dropdown/input yang ON dari config benar-benar menjalankan fiturnya (bukan cuma tampil ON).")
        end
    end)
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddToggle(toggleConfig)
                ...
                -- JANGAN pakai `noSave and nil or (...)`: cabang "true"-nya nil (falsy),
                -- jadi hasilnya selalu jatuh ke cabang `or` dan NoSave tidak pernah berefek.
                local configPath = nil
                if not noSave then
                    configPath = "Toggles." .. title:gsub("%s+", "_")
                end
            ...
            function SectionObject:AddDropdown(dropdownConfig)
                ...
                local configPath = nil
                if not noSave then
                    configPath = (isMulti and "MultiDropdowns." or "Dropdowns.") .. title:gsub("%s+", "_")
                end
                ...
                if isMulti then
                    if default and configPath then
                        ...
                        if current == nil then
                            Library.ConfigSystem.Set(configPath, DeepCopy(default))   -- salin, jangan berbagi ref dgn Default milik user
                        end
                    end
                    ...
                    local flagObj0 = Library.flags[uniqueId]
                    local dropdownObj = {
                        _options = options,
                        Value    = (flagObj0 and flagObj0:GetValue()) or (default or {}),
                        ...
                        GetValue = function(self)
                            local flagObj = Library.flags[uniqueId]
                            if flagObj then return flagObj:GetValue() end
                            return self.Value
                        end,
                    }
                    return dropdownObj
                ...
                if default and configPath then
                    local current = Library.ConfigSystem.Get(configPath, nil)
                    if current == nil then
                        Library.ConfigSystem.Set(configPath, DeepCopy(default))
                    end
                end
                ...
                local dropdownObj = {
                    ...
                    -- Disamakan dengan varian Multi; sebelumnya dropdown single tidak
                    -- punya SetValue/GetValue sama sekali.
                    SetValue = function(self, val)
                        local flagObj = Library.flags[uniqueId]
                        if flagObj and flagObj.SetValue then
                            pcall(function() flagObj:SetValue(val) end)
            ...
            function SectionObject:AddInput(inputConfig)
                ...
                local configPath  = nil
                if not noSave then
                    configPath = "Inputs." .. title:gsub("%s+", "_")
                end
                local inputResult = self._library:CreateInput(self._container, title, configPath, default, callback, placeholder)
                local frame = inputResult and inputResult.frame or inputResult
                if frame then frame.LayoutOrder = getNextLayoutOrder() end
                registerFeature(title, frame, "Input")
                return {
                    _frame = frame,
                    SetValue = function(self, val)
                        if inputResult and inputResult.set then inputResult.set(val) end
                    end,
                    GetValue = function(self)
                        if inputResult and inputResult.get then return inputResult.get() end
                        return nil
                    end,
                }
            end
